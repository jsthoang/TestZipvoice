...
(3) Inference of a single sentence using the Vietnamese model:

python3 -m zipvoice.bin.infer_zipvoice_onnx \
    --onnx-int8 False \
    --model-name zipvoice \
    --prompt-wav prompt_vi.wav \
    --prompt-text "Xin chào, tôi là giọng nói mẫu." \
    --text "Đây là một câu nói được tổng hợp bằng ZipVoice tiếng Việt." \
    --res-wav-path result_vi.wav --lang vi --tokenizer espeak
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="model.pt",
        help="The name of model checkpoint. For the Vietnamese fine-tuned model, "
        "this is typically 'iter-525000-avg-2.pt'.",
    )
...
def main():
    ...
    if params.model_dir is not None:
        ...
        # 1. Try to download text encoder with candidates
        text_encoder_candidates = [
            f"{model_dir}/{text_encoder_name}",
            text_encoder_name,
        ]
        text_encoder_path = hf_try_download(
            HUGGINGFACE_REPO, candidates=text_encoder_candidates
        )
        
        # 2. Try to download FM decoder with candidates
        fm_decoder_candidates = [
            f"{model_dir}/{fm_decoder_name}",
            fm_decoder_name,
        ]
        fm_decoder_path = hf_try_download(
            HUGGINGFACE_REPO, candidates=fm_decoder_candidates
        )
        
        # 3. Try to download config with candidates
        config_candidates = [
        ...
        # 4. Try to download tokens with candidates
        token_candidates = [
